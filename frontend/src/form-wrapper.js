...
export default function FormWrapper({ device }) {
  // Device is a string telling you if it is from ipad or qr
  ...
  useEffect(() => {
    ...
    if (storedName && device === "QR") {
      setInputs(storedName);
    }
  }, []);
  ...
  function scheduleCheck() {
    let schedule = [];
    const signIn = Temporal.Now.plainTimeISO().round({
      smallestUnit: "minute",
    });
    const signInstant = Temporal.Now.instant().round({
      smallestUnit: "minute",
    });
  ...
  function timeCheck(schedule) {
    ...
    switch (schedule.times.findIndex(isSignIn)) {
      case 0:
        rv = {
          reasons: [{ name: "Before or After School", id: 3 }],
          period: 0,
        };
        break;
      case 1:
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="flex items-center justify-center h-screen">
          <div>
            <h2 className="-mt-16 text-sm font-semibold text-gray-600 uppercase transition dark:text-slate-500">
              login page
            </h2>
            <h1 className="-mt-1 text-3xl font-extrabold transition dark:text-slate-200">
              AHS Library
            </h1>
            <div className="flex flex-col h-48 px-4 py-5 mt-2 bg-white border border-gray-200 rounded-lg transition justify-evenly dark:bg-slate-800 dark:border-slate-600 sm:p-5 shadow-sm w-96">
              <div cla="transition -mt-1 mb-3">
                <label
                  htmlFor="fullname"
                  className="block text-sm font-medium text-gray-700 transition dark:text-slate-300"
                >
                  {" "}
                  Full Name{" "}
                </label>
              ...
              <div className="flex flex-row transition shadow-sm dark:bg-slate-700 p-1.5 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-sky-500 dark:focus:border-sky-500 block w-full pr-12 sm:text-sm border-gray-300 dark:border-slate-600 rounded-md dark:text-slate-200">
                ...
                />
                <label
                  htmlFor="sent-by-teacher"
                  className="block w-full pr-12 border-gray-300 transition shadow-sm dark:bg-slate-700 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-sky-500 dark:focus:border-sky-500 sm:text-sm dark:border-slate-600 rounded-md dark:text-slate-200"
                >
                  Sent by Teacher{" "}
                </label>
              </div>
              ...
              <div tw="transition mt-5 border-t border-gray-300 dark:border-slate-600 relative bg-gray-50 dark:bg-slate-700 -mx-5 px-5 py-3 -mb-5 rounded-b-md">
                <div tw="relative flex">
                  ...
                  >
                    <HiUpload
                      className="transition -ml-1.5 mr-1 h-5 w-5 text-gray-400 group-hover:text-blue-200 dark:text-slate-400 dark:group-hover:text-sky-300"
                      aria-hidden="true"
                    />
                    Submit
                  </button>
            ...
            <p className="transition absolute font-semibold text-xs bottom-5 right-5 dark:text-slate-300">
              An AHS Programming Club Project, made by Judah Spiegel, Sajjaad
              Farzad, Arjun Verma, and Kadin Demarche
            </p>
          </div>
        </div>
      </div>
